...
export function FlowStacksOrbitalTimeline() {
  ...
  return (
    <div className="w-full h-[500px] bg-slate-950 rounded-2xl overflow-hidden relative flex items-center justify-center">
      ...
      {/* Orbital Nodes */}
      {flowStacksTimelineData.map((item, index) => {
        const position = calculateNodePosition(index, flowStacksTimelineData.length);
      ...
      <svg className="absolute inset-0 w-full h-full pointer-events-none">
        {flowStacksTimelineData.map((_, index) => {
          const currentPos = calculateNodePosition(index, flowStacksTimelineData.length);
          const nextIndex = (index + 1) % flowStacksTimelineData.length;
          const nextPos = calculateNodePosition(nextIndex, flowStacksTimelineData.length);
      ...
      {selectedNodeId && (
        ...
        >
          {(() => {
            const selectedItem = flowStacksTimelineData.find(item => item.id === selectedNodeId);
...
export default FlowStacksOrbitalTimeline;
